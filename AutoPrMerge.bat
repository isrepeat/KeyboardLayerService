...
:: --- Navigate to current repo directory ---
CD /D "%~dp0"
...
:: --- Get PR number using gh CLI JSON parsing ---
...
:: --- Handle optional new branch logic ---
IF NOT "%NewBranch%"=="" (

    :: Determine current local branch
    FOR /F "delims=" %%b IN ('git branch --show-current') DO SET CurrentBranch=%%b
	::FOR /F "usebackq delims=" %%b IN (`git rev-parse --abbrev-ref HEAD`) DO SET CurrentBranch=%%b
