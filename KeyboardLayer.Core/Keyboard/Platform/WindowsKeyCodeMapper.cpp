#include "WindowsKeyCodeMapper.h"
#include "WindowsKeyCode.h"

namespace Keyboard::Platform {
	std::optional<int> WindowsKeyCodeMapper::ToNative(Keyboard::Core::Enums::LogicalKey key) const {
		using LK = Keyboard::Core::Enums::LogicalKey;
		using WK = Enums::WindowsKeyCode;

		switch (key) {
#define MAP(lk, wk) case LK::lk: return static_cast<int>(WK::wk);
...
#undef MAP
	...
	std::optional<Keyboard::Core::Enums::LogicalKey> WindowsKeyCodeMapper::FromNative(int code) const {
		using LK = Keyboard::Core::Enums::LogicalKey;
		using WK = Enums::WindowsKeyCode;

		switch (static_cast<WK>(code)) {
#define MAP(wk, lk) case WK::wk: return LK::lk;
